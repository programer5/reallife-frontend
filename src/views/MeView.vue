    ...
    <section class="card settingsCard">
      <div class="sectionHead">
        <div>
          ...
          <div class="sub">약속/할일 리마인더를 브라우저와 기기 감각에 맞게 조절해요.</div>
        </div>
      </div>
      <div class="settingGrid">
        <button class="settingItem" type="button" @click="toggleBrowserNotify">
          <div class="settingText">
            ...
            <span>다가오는 리마인더를 시스템 알림으로 받아요.</span>
          </div>
          <span class="settingState" :data-on="settings.pinRemindBrowserNotify">{{ settings.pinRemindBrowserNotify ? 'ON' : 'OFF' }}</span>
        </button>
        <button class="settingItem" type="button" @click="settings.togglePinRemindSound()">
          <div class="settingText">
            <strong>사운드</strong>
            <span>리마인더 도착 시 소리로 알려줘요.</span>
          </div>
          <span class="settingState" :data-on="settings.pinRemindSound">{{ settings.pinRemindSound ? 'ON' : 'OFF' }}</span>
        </button>
        <button class="settingItem" type="button" @click="settings.togglePinRemindVibrate()">
          <div class="settingText">
            <strong>진동</strong>
            <span>지원되는 기기에서 짧은 진동으로 알려줘요.</span>
          </div>
          <span class="settingState" :data-on="settings.pinRemindVibrate">{{ settings.pinRemindVibrate ? 'ON' : 'OFF' }}</span>
        </button>
      </div>
    </section>
...
.settingItem{text-align:left;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px;border-radius:18px;border:1px solid var(--border);background:color-mix(in oklab,var(--surface-2) 76%,transparent);color:var(--text)}
...
.settingState{min-width:58px;height:34px;padding:0 12px;border-radius:999px;display:grid;place-items:center;font-weight:900;border:1px solid var(--border);background:rgba(255,255,255,.04)}
.settingState[data-on="true"]{background:color-mix(in oklab,var(--accent) 16%,transparent);border-color:color-mix(in oklab,var(--accent) 38%,var(--border))}
...
@media (max-width:820px){.heroTop{flex-direction:column}.heroActions{justify-content:flex-start}.stats{grid-template-columns:1fr}.formGrid,.twoCol,.checkGrid,.settingGrid{grid-template-columns:1fr}.avatarLg{width:96px;height:96px;border-radius:22px}}
</style>