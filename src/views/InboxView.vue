...
const filteredItems = computed(() => {
  switch (inboxFilter.value) {
    case 'PRIORITY': return items.value;
...
const filterChips = computed(() => ([
  ...
  { key: 'ACTION', label: `액션 ${actionItems.value.length}` },
]));
const currentFilterLabel = computed(() => filterChips.value.find((c) => c.key === inboxFilter.value)?.label || '전체');
const prioritySummary = computed(() => priorityItems.value[0] ? `${priorityItems.value[0].targetLabel || formatType(priorityItems.value[0].type)}부터 확인하면 가장 빨리 행동으로 이어질 수 있어요.` : '새 흐름이 생기면 서버 우선순위 기준으로 여기에서 가장 먼저 보여줘요.');
const inboxSummaryPills = computed(() => {
  ...
  if (commentItems.value.length) list.push(`댓글 ${commentItems.value.length}`);
  if (reactionItems.value.length) list.push(`반응 ${reactionItems.value.length}`);
  if (actionItems.value.length) list.push(`액션 ${actionItems.value.length}`);
...
function itemStatus(n) {
  if (!n?.read) return '지금 확인';
  switch (categoryOf(n)) {
    case 'REMINDER': return '확인해 둔 리마인더';
    case 'MESSAGE': return '최근 대화 흐름';
    case 'COMMENT': return '댓글 흐름 확인됨';
    case 'REACTION': return '확인된 반응';
    case 'ACTION': return '확인한 액션 변경';
    default: return '확인한 알림';
  }
}
function itemNextAction(n) { return n?.actionHint || '상세 흐름을 열어 다음 행동 정하기'; }
function itemPriorityTone(n) { const p = Number(n?.priorityScore || 0); if (p >= 1400) return 'strong'; if (p >= 1200) return 'warning'; return 'normal'; }
async function refreshNow() { await noti._refreshNow?.(); if (!noti._refreshNow) await noti.refresh(); }
async function markAllRead() { if (busy.value) return; busy.value = true; noti.markAllLocalRead?.(); try { await readAllNotifications(); toast.success('완료', '알림을 모두 읽음 처리했어요.'); noti.refresh?.(); } catch (e) { toast.error('실패', e?.response?.data?.message || '잠시 후 다시 시도해주세요.'); await refreshNow(); } finally { busy.value = false; } }
async function clearRead() { if (cleaning.value) return; cleaning.value = true; try { const res = await clearReadNotifications(); const removedLocal = noti.purgeReadLocal?.() ?? 0; toast.success('정리 완료', `읽은 알림 ${res?.deletedCount ?? removedLocal ?? 0}개를 정리했어요.`); noti.refresh?.(); } catch (e) { toast.error('실패', e?.response?.data?.message || '잠시 후 다시 시도해주세요.'); await refreshNow(); } finally { cleaning.value = false; } }
async function openItem(n) {
  const wasUnread = !n.read; if (wasUnread) noti.markLocalRead?.(n.id);
...
function setupIo() {
  nextTick(() => {
    ...
    io = new IntersectionObserver((entries) => { if (entries[0]?.isIntersecting) noti.loadMore?.(); }, { rootMargin: '120px' });
    io.observe(sentinelEl.value);
  });
}
onMounted(async () => { await noti._refreshNow?.(); setupIo(); });
...
<template>
  <div class="page">
    <header class="head rl-cardish">
      <div>
        <div class="eyebrow">CONNECT · INBOX</div>
        <h1 class="title">지금 이어질 흐름</h1>
        <p class="sub">서버 우선순위 기준으로 리마인더, 메시지, 댓글, 반응을 먼저 정리해 보여줘요.</p>
      </div>
      <div class="actions">
    ...
    <section class="hero rl-cardish">
      <div class="heroMain">
        <div class="heroLabel">SERVER PRIORITY</div>
        <div class="heroTitle">{{ todayPriorityTitle }}</div>
        <div class="heroSub">{{ todayPriorityBody }}</div>
        <div class="heroPills"><span v-for="pill in inboxSummaryPills" :key="pill" class="heroPill">{{ pill }}</span></div>
      </div>
      <div class="heroStats">
    ...
    <section class="priorityRail rl-cardish" v-if="priorityItems.length">
      <div class="priorityRail__head">
        <div><div class="priorityRail__eyebrow">PRIORITY ORDER</div><div class="priorityRail__title">우선 확인 순서</div></div>
        <div class="priorityRail__hint">{{ prioritySummary }}</div>
      </div>
      <div class="priorityRail__list">
        <button v-for="n in priorityItems" :key="n.id" class="priorityMini" :class="`priorityMini--${itemPriorityTone(n)}`" type="button" @click="openItem(n)">
          ...
          <div class="priorityMini__foot">{{ n.actionHint || n.targetLabel || '원본으로 이동' }}</div>
        </button>
      </div>
    </section>
    ...
    <div v-else class="listWrap">
      <section class="summary rl-cardish">
        ...
        <div class="sumMeta">
          <span class="sumChip">리마인더 {{ reminderItems.length }}</span><span class="sumChip">메시지 {{ messageItems.length }}</span><span class="sumChip">댓글 {{ commentItems.length }}</span><span class="sumChip">반응 {{ reactionItems.length }}</span><span class="sumChip">액션 {{ actionItems.length }}</span><span class="sumChip">현재 보기 {{ currentFilterLabel }}</span>
        </div>
        <div class="sumActions"><RlButton size="sm" variant="soft" @click="markAllRead" :disabled="busy || loading || !hasUnread">전체 읽음</RlButton><RlButton size="sm" variant="soft" @click="clearRead" :disabled="cleaning || loading || !items.length">읽은 알림 정리</RlButton></div>
      </section>

      <div class="listHead">
        <div><div class="listTitle">모든 알림</div><div class="listSub">서버가 우선순위와 원본 이동 경로를 함께 계산해서, 바로 다음 행동으로 이어질 흐름을 먼저 보여줘요.</div></div>
        <div class="listGuide">필터를 바꾸면 해당 종류만 서버 우선순위 기준으로 다시 정리해 보여줘요.</div>
      </div>
      ...
      <div v-else class="list">
        <button v-for="n in filteredItems" :key="n.id" class="item rl-cardish" :class="[{ unread: !n.read }, `item--${itemPriorityTone(n)}`]" type="button" @click="openItem(n)">
          ...
          <div class="statusBox">
            ...
            <div class="statusLine"><span class="statusLabel">원본 이동</span><span class="statusValue statusValue--muted">{{ n.targetLabel || '원본으로 이동' }}</span></div>
          </div>
          <div class="line2"><span class="hint">{{ n.targetLabel || '원본 보기' }}</span><span class="arrow">›</span></div>
        </button>
      </div>
      <div ref="sentinelEl" class="sentinel"><div v-if="loadingMore" class="loadingMore">더 불러오는 중…</div><div v-else-if="!hasNext" class="end">여기까지 확인했어요 ✨</div></div>
    </div>
  </div>
</template>

<style scoped>
.page{padding:18px 14px 96px;max-width:980px;margin:0 auto;display:grid;gap:12px}.rl-cardish{border:1px solid color-mix(in oklab,var(--border) 88%,transparent);background:color-mix(in oklab,var(--surface) 86%,transparent);box-shadow:0 18px 60px rgba(0,0,0,.28),0 1px 0 rgba(255,255,255,.06) inset;backdrop-filter:blur(14px)}
