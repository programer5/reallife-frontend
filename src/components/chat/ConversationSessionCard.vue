<template>
  <article class="sessionCard" :data-status="session?.status || 'ACTIVE'">
    ...
    <p v-else-if="!interactivePreview" class="sessionCard__notice">자동 재생이 되지 않도록 잠깐 대기 중이에요. <strong>이 세션에 참여</strong>를 누르면 여기서만 재생을 붙여요.</p>
...
const joinLabel = computed(() => (interactivePreview.value ? '참여 중' : '이 세션에 참여'));
