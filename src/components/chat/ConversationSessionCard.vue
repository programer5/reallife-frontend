<template>
  <article class="sessionCard" :data-status="session?.status || 'ACTIVE'">
    ...
    <div class="sessionCard__participants" v-if="session?.participants?.length">
      <span
    ...
    <p v-if="session?.status === 'ENDED'" class="sessionCard__notice">종료된 세션이에요. 링크는 다시 열 수 있지만 재생 제어는 잠겨 있어요.</p>
    <p v-else-if="!session?.canControl" class="sessionCard__notice">지금은 호스트만 재생을 제어할 수 있어요.</p>
    <div class="sessionCard__actions">
      <button type="button" class="sessionCard__btn" :disabled="busy || !canControl" @click="$emit('play', session)">재생</button>
      <button type="button" class="sessionCard__btn" :disabled="busy || !canControl" @click="$emit('pause', session)">일시정지</button>
      <button type="button" class="sessionCard__btn" :disabled="busy || !canControl" @click="$emit('seek', session)">+15초</button>
