...
export function useConversationDetailDockDerivedState(options = {}) {
  ...
  function pinTimelineEvents(pin) {
    const events = [];
    if (pin?.createdAt) {
      events.push({ key: `created-${pin.pinId || pin.id || ""}`, time: pin.createdAt, label: "액션 생성", tone: "create" });
    }

    const status = String(pin?.status || "").toUpperCase();
    if (status === "DONE" && pin?.updatedAt) {
      events.push({ key: `done-${pin.pinId || pin.id || ""}`, time: pin.updatedAt, label: "완료 처리", tone: "done" });
    } else if ((status === "CANCELED" || status === "CANCELLED") && pin?.updatedAt) {
      events.push({ key: `cancel-${pin.pinId || pin.id || ""}`, time: pin.updatedAt, label: "취소 처리", tone: "cancel" });
    }
  ...
  function feedShareTextForPin(pin) {
    ...
    return [
      ...
      "",
      "#RealLife",
    ].filter((line, idx, arr) => line || idx === arr.length - 2).join("\n");
  }
  ...
  function feedShareMetaForPin(pin) {
    ...
    const chips = [time ? `🕒 ${time}` : "", place ? `📍 ${place}` : "", remind && remind !== "리마인드 없음" ? `⏰ ${remind}` : ""].filter(Boolean);
  ...
  const dockVisibleSummary = computed(() => {
    ...
    return { total: list.length, hidden: Math.max(0, list.length - 4), hasMany: list.length > 4 };
  });

  const dockStatusSummary = computed(() => {
    const now = Date.now();
    const list = Array.isArray(pins.value) ? pins.value : [];
    let overdue = 0; let upcoming = 0; let todoReady = 0; let placeSaved = 0;
    list.forEach((p) => {
      const kind = classifyPin(p);
      const ts = p?.startAt ? new Date(p.startAt).getTime() : 0;
      if (kind === "PROMISE") { if (ts && ts < now) overdue += 1; else upcoming += 1; return; }
      if (kind === "TODO") { todoReady += 1; return; }
  ...
  const timelinePriorityReason = computed(() => {
    if (dockStatusSummary.value.overdue) return { title: `시간 지난 액션 ${dockStatusSummary.value.overdue}개`, description: "약속 시간이나 처리 시점을 다시 확인해 지금 대화를 놓치지 않게 정리하세요." };
    if (reminderDueSoonCount.value) return { title: `24시간 내 리마인드 ${reminderDueSoonCount.value}개`, description: "곧 울릴 리마인드를 미리 보고, 필요하면 시간이나 내용을 바로 다듬는 흐름이 좋아요." };
    if (dockStatusSummary.value.todoReady) return { title: `바로 할 일 ${dockStatusSummary.value.todoReady}개`, description: "짧게 끝낼 수 있는 할 일을 먼저 처리하면 대화 흐름이 훨씬 가벼워져요." };
    return { title: "저장된 액션 흐름 유지 중", description: "약속, 할 일, 장소가 정리돼 있어서 지금은 필요한 카드만 빠르게 훑으면 돼요." };
  });

  const timelineActionPath = computed(() => {
    if (dockStatusSummary.value.overdue) return { title: "카드에서 시간/상태 먼저 수정", description: "아래 액션 카드에서 수정이나 완료 처리를 바로 눌러 흐름을 정리할 수 있어요." };
    if (nextReminderPin.value) return { title: "리마인더 카드에서 바로 확인", description: "다음 리마인더와 오늘 예정 수를 보고 필요한 액션으로 바로 이동하면 돼요." };
    return { title: "아래 액션 카드에서 바로 처리", description: "수정, 완료, 취소, 피드 공유를 카드 안에서 바로 이어갈 수 있게 정리돼 있어요." };
  });
