...
**추천 0) 공동 플레이 MVP(같이 보기 / 같이 듣기)**
- 대화 상세에서 링크 기반 세션 생성
- 세션 카드 표시 + play/pause/seek 동기화
- 종료 후 액션/공유로 연결
...
### B. 시그니처(바이럴/재방문)
- 메시지 상세에서 공동 플레이(같이 보기 / 같이 듣기)
  - 링크 기반 세션 생성
  - 대화 안에서 같이 재생 상태를 맞추고, 이후 액션/공유로 연결
- 메시지 “비밀번호 잠금(락 메시지)”
...
RealLife에는 운영 상태 확인을 위한 **운영 대시보드**가 존재합니다.
...
현재 운영자 여부는 **프론트 Vite env allowlist 방식**으로 판단합니다.
...
1. Health 상태 확인
2. Realtime(SSE) 상태 확인
3. Reminder Scheduler 상태 확인
4. Failed Alert 확인
5. 최근 서버 에러 확인
6. Slack 테스트 알림 확인
